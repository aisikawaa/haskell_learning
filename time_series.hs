...
file2 :: [(Int,Double)]
file2 = [(11, 201.6),(12, 201.6),(13, 201.5),(14, 203.5),(15, 204.9),(16, 207.1),
         (18, 210.5),(20, 208.8)]
...
createTS :: [Int] -> [a] -> TS a
createTS times values = TS completeTimes extendedValllues
  where completeTimes = [minimum times .. maximum times]
        timeValueMap = Map.fromList (zip times values)
        extendedValllues = map (\v -> Map.lookup v timeValueMap) completeTimes
...
meanMaybe vals = if any (== Nothing) vals
  ...
  -- fromJust も Data.Maybeの関数であり、(\(Just x) -> x) に相当する
